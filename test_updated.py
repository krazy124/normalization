...
def style_conversion_compare(original_df, preview_df, column_name, preview_transformed):
    compare_df = pd.DataFrame({
        f"Original: {column_name}": original_df[column_name],
        f"Preview: {column_name}": preview_df[column_name]
    })

    original_col = f"Original: {column_name}"
    preview_col = f"Preview: {column_name}"

    if not preview_transformed:
        return compare_df

    original_values = compare_df[original_col]
    preview_values = compare_df[preview_col]

    missing_mask = original_values.isna()

    unchanged_mask = (
        original_values.notna()
        & preview_values.notna()
        & (original_values.astype(str) == preview_values.astype(str))
    )
    ...
    compare_df["_sort_order"] = 1
    compare_df.loc[unchanged_mask, "_sort_order"] = 0
    compare_df.loc[changed_mask, "_sort_order"] = 1
    compare_df.loc[missing_mask, "_sort_order"] = 2

    compare_df = compare_df.sort_values("_sort_order").drop(
        columns=["_sort_order"]
    )

    def highlight_preview_column(row):
        styles = [""] * len(row)

        original_value = row[original_col]
        preview_value = row[preview_col]
        preview_index = compare_df.columns.get_loc(preview_col)

        if pd.isna(original_value):
            styles[preview_index] = (
            ...
            pd.notna(original_value)
            and pd.notna(preview_value)
            and str(original_value) == str(preview_value)
        ):
            styles[preview_index] = (
    ...
    return compare_df.style.apply(highlight_preview_column, axis=1)
...
with st.container(border=True):
    ...
    with left_panel:
        ...
        if "preview_transformed" not in st.session_state:
            st.session_state.preview_transformed = False
...
with st.container(border=True):
    ...
    with transform_col:
        ...
        if st.button("Convert to Numeric", key="col_convert_numeric", use_container_width=True):
            ...
            st.session_state.preview_transformed = True
        ...
        if st.button("Convert ISO Date Pattern", key="col_iso_date", use_container_width=True):
    ...
    with compare_col:
        ...
        styled_compare_df = style_conversion_compare(
            st.session_state.dirty_df,
            st.session_state.preview_df,
            selected_column,
            st.session_state.preview_transformed
        )

        st.dataframe(styled_compare_df, use_container_width=True, height=450)
